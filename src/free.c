...
static void *free_for_tiny_or_small(void *ptr, int type);
static void *free_for_large(void *ptr);
...
void    free(void *ptr)
{
    ...
    if (g_mm.tiny_zones != NULL)
        ptr = free_for_tiny_or_small(ptr, TINY);

    if (ptr != NULL && g_mm.small_zones != NULL)
        ptr = free_for_tiny_or_small(ptr, SMALL);

    if (ptr != NULL && g_mm.large_zones != NULL)
        ptr = free_for_large(ptr);
